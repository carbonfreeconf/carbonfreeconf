...
class AnswerQuestionView(LoginRequired, View):
    ...
    def post(self, request, answer_id):
        ...
        if next_url is not '':
            ...
            if 'idconf' in self.kwargs:
                return redirect(reverse('qa_index', kwargs={'idconf': self.kwargs['idconf']}))
            else:
...
class QuestionIndexView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if 'idconf' in self.kwargs:
            ...
            answer__isnull=True,confid=self.kwargs['idconf']).select_related('user')\
            .annotate(num_answers=Count('answer', distinct=True),
                      num_question_comments=Count('questioncomment',
                      distinct=True))
            context['totalcount'] = Question.objects.filter(confid=self.kwargs['idconf']).count()
            context['anscount'] = Answer.objects.filter(confid=self.kwargs['idconf']).count()
        else:
            noans = Question.objects.order_by('-pub_date').filter(
        ...
        context['noans'] = noans
        if 'idconf' in self.kwargs:
            context['idconf'] = self.kwargs['idconf']
            context['reward'] = Question.objects.filter(confid=self.kwargs['idconf']).order_by('-reward').filter(
            reward__gte=1)[:10]
        else:
            context['reward'] = Question.objects.order_by('-reward').filter(
                reward__gte=1)[:10]
        question_contenttype = ContentType.objects.get_for_model(Question)
        #print('question_contenttype',question_contenttype)
        ...
        if 'idconf' in self.kwargs:
            context['tags'] = Tag.objects.filter(
                taggit_taggeditem_items__in=items, question__confid=self.kwargs['idconf']).order_by('-id').distinct()[
                              :10]
            self.request.session['idconf'] = self.kwargs['idconf']
        else:
            context['tags'] = Tag.objects.filter(
                taggit_taggeditem_items__in=items).order_by('-id').distinct()[
                              :10]
            self.request.session['idconf'] = ''

        return context

    def get_queryset(self):#if 'errormessage' in kwargs:
        if 'idconf' in self.kwargs:
            queryset = super(QuestionIndexView, self).get_queryset()\
                .filter(confid=self.kwargs['idconf']).select_related('user')\
                .annotate(num_answers=Count('answer', distinct=True),
                          num_question_comments=Count('questioncomment',
                          distinct=True))
        else:
...
class QuestionsSearchView(QuestionIndexView):
    ...
    def get_queryset(self):
        if 'idconf' in self.kwargs:
            result = super(QuestionsSearchView, self).get_queryset().filter(confid=self.kwargs['idconf'])
        else:
...
class QuestionsByTagView(ListView):
    ...
    def get_queryset(self, **kwargs):
        if 'idconf' in self.kwargs:
            return Question.objects.filter(tags__slug=self.kwargs['tag'],confid = self.kwargs['idconf'])
        else:
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if 'idconf' in self.kwargs:
            context['idconf'] = self.kwargs['idconf']
            context['noans'] = Question.objects.order_by('-pub_date').filter(
                tags__name__contains=self.kwargs['tag'],confid = self.kwargs['idconf'], answer__isnull=True)[:10]
            context['reward'] = Question.objects.order_by('-reward').filter(
                tags__name__contains=self.kwargs['tag'],confid = self.kwargs['idconf'],
                reward__gte=1)[:10]
        else:
            context['noans'] = Question.objects.order_by('-pub_date').filter(
...
class CreateQuestionView(LoginRequired, CreateView):
    ...
    def form_valid(self, form):
        ...
        #print('quest',self.kwargs['idconf'])
        if 'idconf' in self.kwargs:#self.kwargs.get('pk')
            print('ok'+self.kwargs['idconf'])
            form.instance.confid = self.kwargs['idconf']
    ...
    def get_success_url(self):
        ...
        if 'idconf' in self.kwargs:
            return reverse('qa_index', kwargs={'idconf':self.kwargs['idconf']})
        else:
...
class UpdateQuestionView(LoginRequired, AuthorRequiredMixin, UpdateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        idconf=question.confid
        print('idc',idconf)
        if idconf==0:
            context['idconf'] = ''
        else:
            context['idconf'] = idconf
        print('idc2',context['idconf'])
        return context
...
class DeleteQuestionView(LoginRequired, AuthorRequiredMixin, DeleteView):
    """
    Delete the question
    """
    template_name = 'qa/update_question.html'
    ...
    def get_success_url(self):
        #question = self.get_object()
        #return reverse('qa_detail', kwargs={'pk': question.pk})
        if 'idconf' in self.kwargs and self.request.session['idconf']!='':
            print('jk')
            return reverse('qa_index', kwargs={'idconf': self.kwargs['idconf']})
        else:
            print('ok')
...
class CreateAnswerView(LoginRequired, CreateView):
    ...
    def form_valid(self, form):
        ...
        idco=Question.objects.filter(id=self.kwargs['question_id']).values_list('confid',flat=True)[0]
        form.instance.confid = idco#self.kwargs['idconf']
